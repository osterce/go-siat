...
type SiatCodigosService interface {
	...
	NotificaCertificadoRevocado(ctx context.Context, config config.Config, req any) (*soap.EnvelopeResponse[codigos.NotificaCertificadoRevocadoResponse], error)

	// SolicitudCufd gestiona la obtención del Código Único de Facturación Diaria para un punto de venta.
	SolicitudCufd(ctx context.Context, config config.Config, req any) (*soap.EnvelopeResponse[codigos.CufdResponse], error)

	// SolicitudCufdMasivo permite la tramitación simultánea de múltiples códigos CUFD.
	SolicitudCufdMasivo(ctx context.Context, config config.Config, req any) (*soap.EnvelopeResponse[codigos.CufdMasivoResponse], error)

	// SolicitudCuis obtiene el Código Único de Inicio de Sistemas necesario para operar ante el SIAT.
	SolicitudCuis(ctx context.Context, config config.Config, req any) (*soap.EnvelopeResponse[codigos.CuisResponse], error)

	// SolicitudCuisMasivo facilita la generación de múltiples códigos CUIS en una sola operación.
	SolicitudCuisMasivo(ctx context.Context, config config.Config, req any) (*soap.EnvelopeResponse[codigos.CuisMasivoResponse], error)

	// VerificarNit valida si un Número de Identificación Tributaria se encuentra activo y habilitado.
	VerificarNit(ctx context.Context, config config.Config, req any) (*soap.EnvelopeResponse[codigos.VerificarNitResponse], error)

	// VerificarComunicacion realiza una prueba de conectividad con el servicio de códigos del SIAT.
	VerificarComunicacion(ctx context.Context, config config.Config, req any) (*soap.EnvelopeResponse[codigos.VerificarComunicacionResponse], error)
}
